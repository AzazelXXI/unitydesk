...
def fix_activity_service():
    ...
    if 'metadata=json.dumps(metadata)' in content:
        content = content.replace('metadata=json.dumps(metadata)', 'extra_data=json.dumps(extra_data)')
    ...
    print("=" * 60)
    print("Changes made:")
    print("1. Fixed parameter name in ActivityService.log_activity: metadata -> extra_data")
