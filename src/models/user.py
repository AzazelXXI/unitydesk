...
class User(Base, BaseModel):
    ...
    primary_department = relationship("Department", foreign_keys=[department_id], back_populates="primary_members")
    position = relationship("Position", back_populates="users")
    manager = relationship("User", remote_side="User.id", backref="direct_reports", foreign_keys=[manager_id])
...
class Department(Base, BaseModel):
    ...
    path = Column(String(255), nullable=True)  # Materialized path for efficient hierarchy queries
    level = Column(Integer, default=0)  # Hierarchy level (0=root, 1=division, etc.)
    
    # Relationships
    parent = relationship("Department", remote_side="Department.id", backref="subdepartments")
    head = relationship("User", foreign_keys=[head_user_id])
    primary_members = relationship("User", back_populates="primary_department", foreign_keys="User.department_id")
    positions = relationship("Position", back_populates="department")
    memberships = relationship("DepartmentMembership", back_populates="department", cascade="all, delete-orphan")
...
class Position(Base, BaseModel):
    ...
    reports_to = relationship("Position", remote_side="Position.id", backref="subordinate_positions")
    users = relationship("User", back_populates="position")


class DepartmentMembership(Base, BaseModel):
