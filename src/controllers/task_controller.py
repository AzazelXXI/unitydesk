...
# from src.models.project import MarketingProject
...
class TaskController:
    ...
    async def get_workflow_steps(
        ...
        try:
            ...
            project_query = select(MarketingProject).filter(
            ...
            query = (
                ...
                .options(joinedload(WorkflowStep.assigned_to))
            ...
            logger.error(
                f"Error fetching workflow steps for project {project_id}: {str(e)}"
            )
            raise HTTPException(
    ...
    async def get_workflow_step(
        project_id: int, step_id: int, db: AsyncSession
    ) -> WorkflowStep:
        """
        ...
        try:
            ...
            query = (
                select(WorkflowStep)
                .filter(
                    WorkflowStep.project_id == project_id, WorkflowStep.id == step_id
                )
                .options(joinedload(WorkflowStep.assigned_to))
            )
            ...
            if not step:
                logger.warning(
                    f"Workflow step with ID {step_id} not found for project {project_id}"
                )
                raise HTTPException(
                    status_code=status.HTTP_404_NOT_FOUND,
            ...
            logger.error(
                f"Error fetching workflow step {step_id} for project {project_id}: {str(e)}"
            )
            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
    ...
    async def update_workflow_step(
        project_id: int, step_id: int, step_data: WorkflowStepUpdate, db: AsyncSession
    ) -> WorkflowStep:
        """
        ...
        try:
            ...
            logger.error(
                f"Error updating workflow step {step_id} for project {project_id}: {str(e)}"
            )
            await db.rollback()
            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
    ...
    async def create_task(
        project_id: int, task_data: MarketingTaskCreate, db: AsyncSession
    ) -> MarketingTask:
        """
        ...
        try:
            ...
            if task_data.workflow_step_id:
                ...
                if not step:
                    logger.warning(
                        f"Workflow step with ID {task_data.workflow_step_id} not found for project {project_id}"
                    )
                    raise HTTPException(
                        status_code=status.HTTP_404_NOT_FOUND,
            ...
            if task_data.parent_task_id:
                ...
                if not parent:
                    logger.warning(
                        f"Parent task with ID {task_data.parent_task_id} not found for project {project_id}"
                    )
                    raise HTTPException(
                        status_code=status.HTTP_404_NOT_FOUND,
    ...
    async def get_task(task_id: int, db: AsyncSession) -> MarketingTask:
        ...
        try:
            ...
            query = (
                ...
                .filter(MarketingTask.id == task_id)
                .options(
                    joinedload(MarketingTask.creator),
                    joinedload(MarketingTask.assignee),
                    joinedload(MarketingTask.workflow_step),
                    ...
                    joinedload(MarketingTask.subtasks).joinedload(
                        MarketingTask.assignee
                    ),
                )
            )
    ...
    async def update_task(
        task_id: int, task_data: MarketingTaskUpdate, db: AsyncSession
    ) -> MarketingTask:
        """
        Update a task
        """
        try:
            ...
            if "status" in update_data and update_data["status"] != task.status:
                ...
                if update_data[
                    "status"
                ] == TaskStatus.COMPLETED and not update_data.get("completed_date"):
            ...
            if "workflow_step_id" in update_data and update_data["workflow_step_id"]:
                ...
                if not step:
                    logger.warning(
                        f"Workflow step with ID {update_data['workflow_step_id']} not found for project {task.project_id}"
                    )
                    raise HTTPException(
                        status_code=status.HTTP_404_NOT_FOUND,
    ...
    async def create_task_comment(
        task_id: int, comment_data: TaskCommentCreate, db: AsyncSession
    ) -> TaskComment:
        """
    ...
    async def update_task_comment(
        ...
        try:
            ...
            if not comment:
                logger.warning(
                    f"Comment with ID {comment_id} not found for task {task_id}"
                )
                raise HTTPException(
                    status_code=status.HTTP_404_NOT_FOUND,
            ...
            if comment_data.user_id != comment.user_id:
                logger.warning(
                    f"User {comment_data.user_id} not authorized to update comment {comment_id}"
                )
                raise HTTPException(
            ...
            logger.error(
                f"Error updating comment {comment_id} for task {task_id}: {str(e)}"
            )
            await db.rollback()
            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
    ...
    async def delete_task_comment(
        task_id: int, comment_id: int, user_id: int, db: AsyncSession
    ) -> None:
        """
        ...
        try:
            ...
            if user_id != comment.user_id:
                logger.warning(
                    f"User {user_id} not authorized to delete comment {comment_id}"
                )
                raise HTTPException(
                    status_code=status.HTTP_403_FORBIDDEN,
            ...
            logger.error(
                f"Error deleting comment {comment_id} from task {task_id}: {str(e)}"
            )
            await db.rollback()
            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
