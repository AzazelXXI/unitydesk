...
def create_department(department: DepartmentCreate, db: Session = Depends(get_db)):
    ...
    if department.parent_id:
        parent = (
            db.query(Department).filter(Department.id == department.parent_id).first()
        )
        if not parent:
            raise HTTPException(
...
def get_departments(
    ...
    return (
        query.order_by(Department.level, Department.parent_id, Department.order_index)
        .offset(skip)
        .limit(limit)
        .all()
    )
...
def update_department(
    ...
    if (
        ...
        and update_data["parent_id"] != db_department.parent_id
    ):
        ...
        if update_data["parent_id"] is not None:
            parent = (
                db.query(Department)
                .filter(Department.id == update_data["parent_id"])
                .first()
            )
            if not parent:
                raise HTTPException(
            ...
            update_data["path"] = (
                f"{parent.path}/{parent.id}" if parent.path else str(parent.id)
            )
            ...
            subdepartments = (
                ...
                .filter(
                    ...
                    if old_path
                    else Department.parent_id == department_id
                )
                .all()
            )
...
def delete_department(department_id: int, db: Session = Depends(get_db)):
    ...
    has_subdepartments = (
        db.query(Department).filter(Department.parent_id == department_id).first()
        is not None
    )
...
@router.get(
    "/{department_id}/members", response_model=List[DepartmentMembershipResponse]
)
def get_department_members(
    department_id: int, is_primary: Optional[bool] = None, db: Session = Depends(get_db)
):
    """Get all members of a department"""
    query = db.query(DepartmentMembership).filter(
...
@router.delete(
    "/{department_id}/members/{user_id}", status_code=status.HTTP_204_NO_CONTENT
)
def remove_department_member(
    department_id: int, user_id: int, db: Session = Depends(get_db)
):
...
def get_subdepartments(
    department_id: int, recursive: bool = False, db: Session = Depends(get_db)
):
    ...
    if recursive:
        ...
        path_query = (
            f"{department.path}/{department_id}%"
            if department.path
            else f"{department_id}/%"
        )
        subdepartments = (
            db.query(Department).filter(Department.path.like(path_query)).all()
        )
...
def get_organizational_chart(
    ...
    counts = (
        ...
        .filter(DepartmentMembership.end_date.is_(None))
    ...
    head_names = {}
    department_heads = (
        db.query(Department.head_user_id, UserProfile.first_name, UserProfile.last_name)
        .join(User, Department.head_user_id == User.id)
        .join(UserProfile, User.id == UserProfile.user_id)
        .filter(Department.head_user_id.isnot(None))
        .all()
    )
    ...
    def build_tree(parent_id=None, level=0):
        ...
        for dept in departments:
            if dept.parent_id == parent_id:
                ...
                head_name = (
                    head_names.get(dept.head_user_id) if dept.head_user_id else None
                )
...
def reorder_departments(
    reorder_request: DepartmentReorderRequest, db: Session = Depends(get_db)
):
    ...
    for item in reorder_request.departments:
        ...
        if item.parent_id != department.parent_id:
            if item.parent_id is not None:
                parent = (
                    db.query(Department).filter(Department.id == item.parent_id).first()
                )
                if not parent:
                    raise HTTPException(
                        status_code=status.HTTP_404_NOT_FOUND,
