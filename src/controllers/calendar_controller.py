...
# Temporarily commenting out model imports and using Any placeholders
...
class CalendarController:
    ...
    async def create_calendar(
        calendar_data: CalendarCreate, db: AsyncSession
    ) -> Calendar:
        """
        ...
        try:
            ...
            logger.info(
                f"Created new calendar: {new_calendar.name} for user {calendar_data.owner_id}"
            )
    ...
    async def get_calendars(
        user_id: int, include_public: bool = True, db: AsyncSession = None
    ) -> List[Calendar]:
        """
    ...
    async def get_calendar(
        calendar_id: int, user_id: int, db: AsyncSession
    ) -> Calendar:
        """
        ...
        try:
            ...
            if calendar.owner_id != user_id and not calendar.is_public:
                logger.warning(
                    f"User {user_id} tried to access calendar {calendar_id} without permission"
                )
                raise HTTPException(
    ...
    async def update_calendar(
        calendar_id: int, calendar_data: CalendarUpdate, user_id: int, db: AsyncSession
    ) -> Calendar:
        """
        ...
        try:
            ...
            if calendar.owner_id != user_id:
                logger.warning(
                    f"User {user_id} tried to update calendar {calendar_id} without permission"
                )
                raise HTTPException(
                    status_code=status.HTTP_403_FORBIDDEN,
    ...
    @staticmethod
    async def delete_calendar(calendar_id: int, user_id: int, db: AsyncSession) -> bool:
        """
        Delete a calendar
        """
        try:
            ...
            if calendar.owner_id != user_id:
                logger.warning(
                    f"User {user_id} tried to delete calendar {calendar_id} without permission"
                )
                raise HTTPException(
                    status_code=status.HTTP_403_FORBIDDEN,
    ...
    async def create_event(
        calendar_id: int, event_data: EventCreate, user_id: int, db: AsyncSession
    ) -> Event:
        """
        ...
        try:
            ...
            if calendar.owner_id != user_id and not calendar.is_public:
                logger.warning(
                    f"User {user_id} tried to add event to calendar {calendar_id} without permission"
                )
                raise HTTPException(
                    status_code=status.HTTP_403_FORBIDDEN,
            ...
            logger.info(
                f"Created new event: {new_event.title} in calendar {calendar_id}"
            )
    ...
    async def update_event(
        event_id: int, event_data: EventUpdate, user_id: int, db: AsyncSession
    ) -> Event:
        """
        ...
        try:
            ...
            if event.organizer_id != user_id:
                logger.warning(
                    f"User {user_id} tried to update event {event_id} without permission"
                )
                raise HTTPException(
                    status_code=status.HTTP_403_FORBIDDEN,
    ...
    @staticmethod
    async def delete_event(event_id: int, user_id: int, db: AsyncSession) -> bool:
        """
        Delete an event
        """
        try:
            ...
            if event.organizer_id != user_id:
                logger.warning(
                    f"User {user_id} tried to delete event {event_id} without permission"
                )
                raise HTTPException(
                    status_code=status.HTTP_403_FORBIDDEN,
    ...
    async def update_participant_status(
        ...
        try:
            ...
            if not participant:
                logger.warning(
                    f"User {user_id} is not a participant in event {event_id}"
                )
                raise HTTPException(
                    status_code=status.HTTP_404_NOT_FOUND,
            ...
            logger.info(
                f"Updated participant status for user {user_id} in event {event_id}"
            )
            return participant
            ...
            logger.error(
                f"Error updating participant status for user {user_id} in event {event_id}: {str(e)}"
            )
            await db.rollback()
            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
