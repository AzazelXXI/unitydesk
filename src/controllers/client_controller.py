...
class ClientController:
    ...
    @staticmethod
    async def get_client(client_id: int, db: AsyncSession) -> Client:
        """
    ...
    @staticmethod
    async def update_client(client_id: int, client_data: ClientUpdate, db: AsyncSession) -> Client:
        """
    ...
    async def create_client_contact(
        ...
        try:
            ...
            logger.info(f"Created new contact {new_contact.name} for client {client_id}")
            return new_contact
        except HTTPException:
            raise
        except Exception as e:
    ...
    @staticmethod
    async def get_client_contacts(client_id: int, db: AsyncSession) -> List[ClientContact]:
        """
    ...
    async def get_client_contact(
        ...
        try:
            ...
            if not contact:
                logger.warning(f"Contact with ID {contact_id} not found for client {client_id}")
                raise HTTPException(
                    status_code=status.HTTP_404_NOT_FOUND,
            ...
            logger.error(f"Error fetching contact {contact_id} for client {client_id}: {str(e)}")
            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
    ...
    async def update_client_contact(
        ...
        try:
            # Get the contact
            contact = await ClientController.get_client_contact(client_id, contact_id, db)
            ...
            logger.error(f"Error updating contact {contact_id} for client {client_id}: {str(e)}")
            await db.rollback()
            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
    ...
    @staticmethod
    async def delete_client_contact(client_id: int, contact_id: int, db: AsyncSession) -> None:
        """
        ...
        try:
            ...
            logger.error(f"Error deleting contact {contact_id} from client {client_id}: {str(e)}")
            await db.rollback()
            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
    ...
    async def _unset_primary_contacts(client_id: int, db: AsyncSession) -> None:
        ...
        """
        try:
            query = update(ClientContact).where(
                ClientContact.client_id == client_id,
            ...
            logger.error(f"Error unsetting primary contacts for client {client_id}: {str(e)}")
            raise
