...
class UserTypeService:
                ...
                user_types.append(
                    {
                        ...
                        or f"Custom {custom_type.display_name} role",
                    }
                )
    ...
    async def create_custom_user_type(
        db: AsyncSession, type_name: str, display_name: str, description: str = None
    ) -> CustomUserType:
        ...
        try:
            ...
            if type_name.lower().replace(" ", "_") in DEFAULT_USER_TYPES:
                raise ValueError(
                    f"User type '{type_name}' conflicts with default types"
                )

            custom_type = CustomUserType(
                type_name=type_name, display_name=display_name, description=description
            )
    ...
    async def delete_custom_user_type(db: AsyncSession, type_id: int) -> bool:
        ...
        try:
            ...
            result = await db.execute(
                users_with_type_query, {"user_type": custom_type.type_name}
            )
            ...
            if user_count > 0:
                ...
                await db.commit()
                logger.info(
                    f"Deactivated custom user type: {custom_type.type_name} (used by {user_count} users)"
                )
                return False
            else:
                # Safe to delete
    ...
    async def get_user_type_display_name(db: AsyncSession, type_name: str) -> str:
        ...
        try:
            ...
            query = select(CustomUserType).where(
                CustomUserType.type_name == type_name, CustomUserType.is_active == True
            )
            result = await db.execute(query)
            custom_type = result.scalar_one_or_none()
    ...
    @staticmethod
    async def initialize_default_types(db: AsyncSession) -> bool:
        """
        Initialize default user types in the database if they don't exist.
        This is useful for ensuring the custom_user_types table has default entries.
        """
        try:
            # Check if custom_user_types table exists and is accessible
            # If the table doesn't exist, this will fail gracefully
            query = select(CustomUserType).limit(1)
            await db.execute(query)

            logger.info(
                "Custom user types table is accessible - default types are managed via code"
            )
            return True

        except Exception as e:
            logger.info(f"Custom user types table not available: {str(e)}")
            logger.info(
                "Using default types from code - this is normal for basic setups"
            )
            return False

    @staticmethod
    async def get_user_type_details(
        ...
        try:
            ...
            if type_name in DEFAULT_USER_TYPES:
                return {
                    ...
                    "description": f'Default {type_name.replace("_", " ").title()} role',
                    "is_custom": False,
                }
            ...
            logger.error(f"Error getting user type details: {str(e)}")
