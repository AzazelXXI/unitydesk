...
export const initializeMedia = async () => {
    try {
        ...
        if (
            ...
            alert(
                "Permission denied: Please allow access to camera and microphone in your browser settings."
            );
        } else if (error.name === "NotFoundError") {
            alert(
                "No camera or microphone found. Please check your hardware connections."
            );
        } else if (
            ...
            alert(
                "Cannot satisfy the requested media constraints. Try with different settings."
            );
        } else if (error.name === "TypeError") {
            alert("Invalid media constraints specified.");
        } else {
            alert("Failed to access camera or microphone: " + error.message);
        }
...
export const toggleAudio = () => {
    ...
    if (audioTrack) {
        ...
        console.log(
            `Microphone ${audioTrack.enabled ? "enabled" : "disabled"}`
        );
    } else {
...
export const toggleVideo = () => {
    ...
    if (videoTrack) {
        ...
        // Update title/tooltip instead of changing text
        videoToggleBtn.title = videoTrack.enabled
            ? "Turn Off Camera"
            : "Turn On Camera";
...
export const stopScreenShare = async () => {
    try {
        ...
        const originalVideoTrack =
            state.originalVideoStream.getVideoTracks()[0];
