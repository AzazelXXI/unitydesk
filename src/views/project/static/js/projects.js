...
function animateStatsCards() {
  ...
  statsCards.forEach((card, index) => {
    ...
    if (numberElement) {
      const finalValue = parseInt(numberElement.textContent.replace(/[^\d]/g, '')) || 0;
...
function searchProjects(searchTerm) {
  ...
  projectCards.forEach(card => {
    const projectName = card.querySelector('.project-title a').textContent.toLowerCase();
...
function updateStatsDisplay(stats) {
  ...
  const statTypes = ['total', 'planning', 'in_progress', 'completed', 'total_budget', 'avg_progress'];
  ...
  statsCards.forEach((card, index) => {
    ...
    if (numberElement && statTypes[index]) {
      ...
      const currentValue = parseInt(numberElement.textContent.replace(/[^\d]/g, '')) || 0;
