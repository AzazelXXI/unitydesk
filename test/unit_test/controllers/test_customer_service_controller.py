...
# from src.models_backup.customer_service import ( # Commented out
#     ServiceTicket,
#     ServiceStep,
#     TicketStep,
#     QuoteRequest,
...
#     StepStatus,
#     DocumentType,
#     TicketStatus, # Assuming TicketStatus was also from here
# )
...
class TestTicketManagement:
    ...
    async def test_create_service_ticket(self, mock_db, mock_ticket):
        ...
        with patch(
            "src.controllers.customer_service_controller.CustomerServiceController.create_service_ticket",
            side_effect=mocked_create_ticket,
        ):

            # Act
            result = await CustomerServiceController.create_service_ticket(
                ticket_data, mock_db
            )
    ...
    async def test_update_service_ticket(self, mock_db, mock_ticket):
        ...
        with patch(
            "src.controllers.customer_service_controller.CustomerServiceController.update_service_ticket",
            side_effect=mocked_update_ticket,
        ):

            # Act
            result = await CustomerServiceController.update_service_ticket(
                1, ticket_data, mock_db
            )
    ...
    async def test_delete_service_ticket_success(self, mock_db, mock_ticket):
        ...
        # Use string path to patch instead of direct object reference
        with patch(
            "src.controllers.customer_service_controller.CustomerServiceController.delete_service_ticket",
...
class TestServiceSteps:
    ...
    async def test_create_service_step(self, mock_db, mock_step):
        ...
        with patch.dict(
            "sys.modules", {"src.controllers.task_controller": MagicMock()}
        ):
            # Now we can safely import
            from src.controllers.customer_service_controller import (
            ...
            with patch(
                "src.controllers.customer_service_controller.ServiceStep",
                return_value=new_step,
            ):

                # Act
                result = await CustomerServiceController.create_service_step(
                    step_data, mock_db
                )
    ...
    async def test_update_service_step(self, mock_db, mock_step):
        ...
        scalar_mock = AsyncMock()
        scalar_mock.scalar_one_or_none = MagicMock(
            return_value=mock_step
        )  # Regular MagicMock
        mock_db.execute.return_value = scalar_mock

        # Act
        result = await CustomerServiceController.update_service_step(
            1, step_data, mock_db
        )
...
class TestTicketSteps:
    ...
    @pytest.mark.asyncio
    async def test_add_step_to_ticket(
        self, mock_db, mock_ticket, mock_step, mock_ticket_step
    ):
        ...
        # Use string path to patch instead of direct object reference
        with patch(
            "src.controllers.customer_service_controller.CustomerServiceController.add_step_to_ticket",
            side_effect=mocked_add_step,
        ):
            # Act
            result = await CustomerServiceController.add_step_to_ticket(
                1, step_data, mock_db
            )
    ...
    async def test_get_ticket_step(self, mock_db, mock_ticket_step):
        ...
        # Use string path to patch instead of direct object reference
        with patch(
            "src.controllers.customer_service_controller.CustomerServiceController.get_ticket_step",
    ...
    async def test_update_ticket_step(self, mock_db, mock_ticket, mock_ticket_step):
        ...
        # Use string path to patch instead of direct object reference
        with patch(
            "src.controllers.customer_service_controller.CustomerServiceController.update_ticket_step",
            side_effect=mocked_update_ticket_step,
        ):

            # Act
            result = await CustomerServiceController.update_ticket_step(
                1, 1, step_data, mock_db
            )
...
class TestQuoteGeneration:
    ...
    async def test_generate_quote_pdf(self, mock_db, mock_ticket, mock_quote_document):
        ...
        with patch.dict(
            ...
            with patch(
                "src.controllers.customer_service_controller.CustomerServiceController.get_service_ticket",
    ...
    async def test_update_quote_document(self, mock_db, mock_quote_document):
        ...
        scalar_mock = AsyncMock()
        scalar_mock.scalar_one_or_none = MagicMock(
            return_value=mock_quote_document
        )  # Not a coroutine
        mock_db.execute.return_value = scalar_mock
