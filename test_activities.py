    ...
    async with async_session() as db:
        try:
            ...
            project_result = await db.execute(
                text("SELECT id, name FROM projects LIMIT 1")
            )
            ...
            if not project_row or not user_row:
                print(
                    "❌ ERROR: Need at least one project and one user in the database!"
                )
                return
            ...
            try:
                ...
                await ActivityService.log_activity(
                    ...
                    extra_data={"task_name": "Test Task", "test_flag": True},
                )
            ...
            try:
                recent_activities = await ActivityService.get_recent_activities(
                ...
                for activity in recent_activities:
                    ...
                    print(
                        f"    User: {activity.user.name if activity.user else 'Unknown'}"
                    )
            ...
            for i, activity_data in enumerate(test_activities):
                try:
                    ...
                    print(
                        f"✅ Inserted test activity: {activity_data['activity_type']}"
                    )
                except Exception as e:
            ...
            print(
                f"✅ Final count: {len(final_activities)} activities for project {project_id}"
            )

            print(f"\n8. Testing web route compatibility...")
            # Test the same query that's used in project_routes.py
            route_activities = await ActivityService.get_recent_activities(
            ...
            for activity in route_activities[:3]:  # Check first 3
                ...
                has_description = (
                    hasattr(activity, "description") and activity.description
                )
                has_created_at = hasattr(activity, "created_at") and activity.created_at

                print(f"  Activity {activity.id}:")
                print(
                    f"    - Has icon: {has_icon} ({getattr(activity, 'icon', 'None')})"
                )
                print(
                    f"    - Has color: {has_color} ({getattr(activity, 'color', 'None')})"
                )
                ...
                if not all(
                    [has_icon, has_color, has_user, has_description, has_created_at]
                ):
            ...
            print(
                f"3. If activities still don't show, check the browser console for errors"
            )
            print(
                f"4. Verify the project_routes.py is passing activities correctly to templates"
            )
